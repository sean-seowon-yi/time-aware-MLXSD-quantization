...
Phase 1 — Generate images
    Reads prompts from evaluation_set.txt (seed<TAB>prompt format), generates
    N images and saves to output_dir/images/.  ``--resume`` skips images whose
    PNG already exists.
...
Usage
-----
    # Step 1 — FP16 baseline (Phase 2 pipeline settings, for paired metrics)
    python -m src.benchmark.benchmark_model \\
        --config fp16_p2 --num-images 150 --reload-n 1 \\
        --output-dir benchmark_results/fp16_p2 --resume

    # Step 2 — W4A8 quantized: generate + compute all metrics
    python -m src.benchmark.benchmark_model \\
    ...
    python -m src.benchmark.benchmark_model \\
        --skip-generation \\
        --generated-dir benchmark_results/w4a8_l2_a0.50_gs32/images \\
        --baseline-dir results/fp16 \\
        --output-dir benchmark_results/w4a8_l2_a0.50_gs32
"""
...
def generate_images(
    ...
    ----------
    img_digits : int
        Zero-pad width for image filenames (e.g. 3 → 000.png, 4 → 0000.png).
        Phase 2 configs (w4a8, fp16_p2) use 3 to match the sweep convention.
    ...
    for img_idx, prompt in pbar:
        ...
        if resume and img_path.exists():
            ...
            if saved:
                ...
                print(f"  [resume] skipping {img_path.name} "
...
def main() -> None:
    ...
    parser.add_argument("--quantized-dir", type=Path, default=None,
                        help="Phase 2 W4A8 quantized model directory (use with --config w4a8)")
    ...
    parser.add_argument("--prompt-csv", type=Path, default=None,
                        help="Prompt file: CSV with 'prompt' column, or tab-separated "
                             "seed<TAB>prompt .txt (default: src/settings/evaluation_set.txt)")
    parser.add_argument("--num-images", type=int, default=150,
                        help="Number of images to generate (default: 150)")
    parser.add_argument("--num-steps", type=int, default=30,
                        help="Denoising steps per image (default: 30)")
    parser.add_argument("--cfg-scale", type=float, default=4.0,
                        help="CFG guidance weight (default: 4.0)")
    ...
    benchmark = {
        ...
        "cfg_scale": args.cfg_scale,
        "seed": args.seed,
        "quantized_dir": str(args.quantized_dir) if args.quantized_dir else None,
