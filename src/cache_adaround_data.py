...
def main():
    ...
    for img_idx in selected_image_ids:
        ...
        # but still encode text to save pooled embeddings if missing.
        pooled_dir = output_dir / "pooled"
        pooled_path = pooled_dir / f"{img_idx:04d}.npz"
        if args.resume:
            remaining_steps = [s for s in key_timesteps if (img_idx, s) not in done_samples]
            if not remaining_steps and pooled_path.exists():
        ...
        try:
            ...
            # Save pooled embedding so AdaRound optimizer can compute correct
            # per-image modulation params during optimization.
            pooled_dir = output_dir / "pooled"
            pooled_dir.mkdir(exist_ok=True)
            np.savez_compressed(
                pooled_dir / f"{img_idx:04d}.npz",
        ...
        for step_idx in key_timesteps:
            ...
            try:
                ...
                # Write per-sample npz (include sigma for polynomial clipping)
                flat = pack_sample(block_data)
                flat["__sigma__"] = np.array(data["sigma"]).reshape(1)
