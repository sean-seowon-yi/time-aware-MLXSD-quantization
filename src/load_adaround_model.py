...
# ---------------------------------------------------------------------------
# Polynomial clipping utilities
# ---------------------------------------------------------------------------

def eval_poly_alpha(coeffs: list, sigma: float, sigma_range: list = None) -> float:
    """Evaluate polynomial clipping range α(σ) = polyval(coeffs, sigma).

    Clamps sigma to the calibration range to prevent extrapolation, and
    clamps the result to be positive (negative alpha is nonsensical).
    """
    if sigma_range is not None:
        sigma = max(sigma_range[0], min(sigma, sigma_range[1]))
    alpha = float(np.polyval(coeffs, sigma))
    return max(alpha, 1e-8)


def load_poly_schedule(path) -> Optional[Dict]:
    """Load polynomial clipping schedule JSON. Returns None if path is None."""
    if path is None:
        return None
    with open(path) as f:
        return json.load(f)
...
class _ActQuantLayer:
    ...
    def _get_scale_and_bits(self):
        ...
        # Fallback: static per-timestep config
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        if scale is None or scale < 1e-8:
            # No poly or static config — check static fallback for shift/outlier
            cfg = self.per_timestep.get(str(self.current_step_key))
            if cfg is None:
                return self.layer(x)
            if cfg["scale"] < 1e-8:
        ...
        shift = None
        if isinstance(cfg, dict) and "shift" in cfg:
...
def apply_act_quant_hooks(
    ...
    proxies : list of _ActQuantLayer
        Set proxy.current_step_key and proxy.current_sigma before each step.
...
def main() -> None:
    ...
    poly_schedule = load_poly_schedule(args.poly_schedule)
    if poly_schedule is not None:
        n_poly = len(poly_schedule.get("layers", {}))
