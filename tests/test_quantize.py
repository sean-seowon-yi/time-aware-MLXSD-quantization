"""Tests for src.phase2.quantize — A8 fake quant, W4A8Linear, model quantization."""
...
class TestFakeQuantizeA8:

    def test_output_shape(self):
        x = mx.array(np.random.randn(4, H).astype(np.float32))
        x_hat = fake_quantize_a8(x)
        mx.eval(x_hat)
        assert x_hat.shape == x.shape

    def test_error_bounded_by_half_scale(self):
        x = mx.array(np.random.randn(8, H).astype(np.float32))
        x_hat = fake_quantize_a8(x)
        mx.eval(x_hat)

        x_np = np.array(x)
        x_hat_np = np.array(x_hat)
        scale = np.max(np.abs(x_np)) / 127.0

        max_err = np.max(np.abs(x_np - x_hat_np))
        assert max_err <= 0.5 * scale + 1e-7
    ...
    def test_small_input_preserved(self):
        x = mx.array(np.full((2, H), 1e-10, dtype=np.float32))
        x_hat = fake_quantize_a8(x)
        mx.eval(x_hat)
        np.testing.assert_allclose(np.array(x_hat), np.array(x), atol=1e-7)

    def test_symmetric_range(self):
        """Output should be within [-128*scale, 127*scale]."""
        x = mx.array(np.random.randn(16, H).astype(np.float32) * 5.0)
        x_hat = fake_quantize_a8(x)
        mx.eval(x_hat)

        x_np = np.array(x)
        scale = max(np.max(np.abs(x_np)) / 127.0, 1e-8)
        assert np.max(np.array(x_hat)) <= 127.0 * scale + 1e-6
        assert np.min(np.array(x_hat)) >= -128.0 * scale - 1e-6
...
class TestW4A8Linear:
    ...
    def _make_w4a8(self, with_b_inv=False, d_in=H, d_out=H):
        ...
        return W4A8Linear(qlinear, b_inv)
    ...
    def test_b_inv_changes_output(self):
        ...
        layer_no = W4A8Linear(qlinear, b_inv=None)
        layer_yes = W4A8Linear(qlinear, b_inv=mx.ones((H,)) * 2.0)
...
class TestQuantizeModel:
    ...
    def test_replaces_layers(self, mock_mmdit, registry, test_config):
        ...
        for entry in registry:
            ...
            assert isinstance(getattr(parent, attr), W4A8Linear), (
                f"{entry['name']} should be W4A8Linear"
            )

        assert len(layer_meta) > 0

    def test_excludes_context_embedder(self, mock_mmdit, registry, test_config):
        quantize_model(mock_mmdit, registry, {}, test_config)
    ...
    def test_layer_meta_has_correct_info(self, mock_mmdit, registry, test_config):
        ...
        meta = quantize_model(mock_mmdit, registry, b_inv_map, test_config)
